...
protected:
	...
	// Functions added
	BinaryNode<Key, Object> *insert(BinaryNode<Key, Object> *sub_root, const Key key, const Object &obj);
	BinaryNode<Key, Object> *erase(BinaryNode<Key, Object> *sub_root, const Key &key);
	int leaf_count(BinaryNode<Key, Object> *sub_root) const;
...
template <typename Key, typename Object>
BinaryNode<Key, Object> *BinaryTree<Key, Object>::insert(BinaryNode<Key, Object> *sub_root, const Key key, const Object &obj)
{
	if (sub_root == nullptr)
	{
		sub_root = new BinaryNode<Key, Object>(key, obj);
	}
	else if (key < sub_root->key)
	{
		sub_root->left = insert(sub_root->left, key, obj);
	}
	else if (key > sub_root->key)
	{
		sub_root->right = insert(sub_root->right, key, obj);
	}
	return sub_root;
}
...
// Public erase
template <typename Key, typename Object>
void BinaryTree<Key, Object>::erase(const Key &key)
{
	if (root != nullptr)
	{
		root = erase(root, key);
...
// Private recursive erase
template <typename Key, typename Object>
BinaryNode<Key, Object> *BinaryTree<Key, Object>::erase(BinaryNode<Key, Object> *sub_root, const Key &key)
{
	if (sub_root == nullptr)
	{
		return nullptr;
	...
	if (key < sub_root->key)
	{
		sub_root->left = erase(sub_root->left, key);
	}
	else if (key > sub_root->key)
	{
		sub_root->right = erase(sub_root->right, key);
	}
	else // key == sub_root->key
	{
		// Case: two children
		if (sub_root->left != nullptr && sub_root->right != nullptr)
		{
			// find minimum in right subtree
			BinaryNode<Key, Object>* minNode = sub_root->right;
			while (minNode->left != nullptr)
				minNode = minNode->left;

			sub_root->key = minNode->key;
			sub_root->obj = minNode->obj;
			sub_root->right = erase(sub_root->right, minNode->key);
		}
		else
		{
			// Case: one child or leaf
			BinaryNode<Key, Object> *discard = sub_root;
			sub_root = (sub_root->left != nullptr) ? sub_root->left : sub_root->right;
			delete discard;
		}
	}
...
template <typename Key, typename Object>
int BinaryTree<Key, Object>::leaf_count() const
{
	return leaf_count(root);
}

template <typename Key, typename Object>
int BinaryTree<Key, Object>::leaf_count(BinaryNode<Key, Object> *sub_root) const
{
	if (sub_root == nullptr)
		return 0;
	...
	{
		return leaf_count(sub_root->left) + leaf_count(sub_root->right);
	}
}
